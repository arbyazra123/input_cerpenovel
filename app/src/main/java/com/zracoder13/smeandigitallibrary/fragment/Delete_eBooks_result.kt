...
import com.zracoder13.smeandigitallibrary.Adapter.Delete_ebooks_adapter
 ...
 *
 */
class Delete_eBooks_result : Fragment() {
    internal lateinit var recyclerView : RecyclerView
    internal lateinit var swipeRefreshLayout : SwipeRefreshLayout
    var Lib_list: List<Ebook_list>? = null
    internal lateinit var Adapter: Delete_ebooks_adapter
    var kategori_item : String = ""
    internal lateinit var manager: RecyclerView.LayoutManager
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
                              savedInstanceState: Bundle?): View? {
        // Inflate the layout for this fragment
        val viewroot =  inflater.inflate(R.layout.fragment_delete_e_books_result, container, false)

        Toast.makeText(context,"Mencari...", Toast.LENGTH_SHORT).show()

        recyclerView = viewroot.findViewById(R.id.delete_ebooks_rv)
...
Toast.makeText(context,"Koneksi Internet Bermaslah",Toast.LENGTH_SHORT).show()
            }

            override fun onResponse(call: Call<List<Ebook_list>>?, response: Response<List<Ebook_list>>?) {
                Lib_list = response!!.body()
                Adapter = Delete_ebooks_adapter(context,Lib_list)
